...
for ind = 3:1:17
    ...
    data_load   = load(nameSTR);
    data_load   = data_load.IterData;
    ...
    equilibrium_positions       = data_load.Trajectories(:, 1);
    trajectory                  = data_load.Trajectories;

    %this will select the desired endpoints and alpha value
    state                       = ind;  
    eq_pos                      = equilibrium_positions;
    ...
    alpha           = data_load.AlphaValues(ind); disp(['Alpha= ', num2str(alpha)])  %from eq_pos file!!!!
    ...
    hold on
    plot(S, VS - min(VS), '.-')
    plot(Sq, VS_interpolate - min(VS_interpolate), 'o')
    hold off
...
%     dE2(ind) = E2(2) - E2(1);
    ...
    velocity = f_trajectory_diff(N_division, trajectory, z_time, particle_n);
%
    %using the previous differentiated curve to form normed vectors
    unit_velocity_e = f_velocity_unit_vector(velocity, particle_n);

    %creating a perpendicular vector to e named f
    %Normalization and Nominator are here for error checking purposes
    [f, delta_e, Normalization, Nominator] = f_f_vector(unit_velocity_e, particle_n);
    ...
    Rot_mtx = f_rotation_matrix(N_division, unit_velocity_e, f, delta_phi, particle_n);

    %Determining the basis vectors from the equilibrium positions and
    %potentials
    [Tau01, EigVal] = f_init_tau(trajectory, alpha, velocity, eta, particle_n);

    %using the rotation matrix above we can create the tau vector basis that
    %goes along the trajectory
    tauspace = f_tau_space(Rot_mtx, Tau01, particle_n);

    %determining the curvature of the trajectory
    C = f_curvature(unit_velocity_e, tauspace, z_time, velocity, particle_n);
    %
    %B matrix -- See notes what this is
    B_mtx = f_B_matrix(trajectory, alpha, tauspace, eta, particle_n);
    ...
    omega_sq = f_omega_squared(B_mtx, velocity, C, particle_n);     

    omega_sq_old = omega_sq;
    for i = 1:particle_n-1
        for j = 1:particle_n-1
            omega_sq(i,j,:) = flip(omega_sq(i,j,:));
        end
    end
    ...
    [Xi_heun, temp_mtx] = f_diff_equation(z_time, omega_sq, r, particle_n);

    %constructing the full propagotor with the 1D term
    [propagator, Trace, Trace2] = f_prefactor(Xi_heun, EigVal, z_time, z_time, alpha, r, particle_n);


    [splittt, action, ODPM1, ODPL, ODPI, ODPM2, ODPM3] = f_action(eta, alpha, trajectory, r, z_time, EigVal, propagator, S, VS, 1, particle_n);
