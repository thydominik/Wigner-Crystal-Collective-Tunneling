...
elseif Method == 3
    addpath('D:\BME PhD\.Wigner Crystal Collective Tunneling\CollectiveTunneling\4 - Trajectory Calculation\3 particle\Simplified MC Data')
elseif Method == 2
    addpath('D:\BME PhD\.Wigner Crystal Collective Tunneling\CollectiveTunneling\4 - Trajectory Calculation\3 particle\Restricted MC Data')
end
...
for ind = 1:16
    ...
    name = ind;
    if Method == 1
        nameSTR = ['Traj_3P_STDMC' num2str(name)];
    elseif Method == 2
        nameSTR = ['Traj_1P_SIMPLEMC' num2str(name)];
    elseif Method == 3
        nameSTR = ['Traj_3P_RESTMC' num2str(name)];
    end
    ...
    NoNP = 500; %The Number of New Points in V(S) (to continue the potential)
    SS = Sq;
    for i = 1:NoNP
    ...
    if FS
        ...
        clf(figure(3))
        hold on
        title('Interpolation of the Effective potential')
...
if Method == 1
    save('StandardSplitting', 'SPLITTINGS')
elseif Method == 2
    save('RestrictedSplitting', 'SPLITTINGS')
elseif Method == 3
    save('SimplifiedSplitting', 'SPLITTINGS')
end
%%
SIMPMC = load('SimplifiedSplitting.mat');
STDMC = load('StandardSplitting.mat');
RESTMC = load('RestrictedSplitting.mat');

ED = load('EDSplitting_3_particles_restricted_Nx_80_beta_1e-05.mat');
ED = ED.data;
ED_a = ED(:, 1);
ED_d = ED(:, 2);
...
plot(STDMC.SPLITTINGS(:, 1), STDMC.SPLITTINGS(:, 2), '.-', 'LineWidth', 2, 'DisplayName', '1D part ED * N-1 Milnikov - STD')
plot(RESTMC.SPLITTINGS(:, 1), RESTMC.SPLITTINGS(:, 2), '.-', 'LineWidth', 2, 'DisplayName', '1D part ED * N-1 Milnikov - REST')
plot(SIMPMC.SPLITTINGS(:, 1), SIMPMC.SPLITTINGS(:, 2), '.-', 'LineWidth', 2, 'DisplayName', '1D part ED * N-1 Milnikov - SIMP')
...
xlim([5 12])
ylim([10^-5 2])
xline(7.44, 'r', 'DisplayName', 'Elbow point')
