function [prop, Trace1, Trace2, Integ1, Integ2, Integ3] = Prefactor(Xi, Omega, EigVals, z, dz, Alpha, R, NoP, Nt)
    ...
    [Fit, gof] = Fitting_Trace(z.', Trace1.')
    func = (Fit.a1 + 0.1) * exp(-((z - Fit.b1)/Fit.c1).^2);
    plot(z, func - min(func));
    plot(z, R./(1-z.^2) .* (func - min(func)));
    plot(z, R./(1-z.^2) .* Trace1)
    plot(z, Trace1)
    xline(z(25))
    hold off
    Trace1 = func - min(func);
