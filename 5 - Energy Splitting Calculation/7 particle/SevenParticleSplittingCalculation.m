...
%     NoP = length(Trajectory(1, :));
    ...
    if FS
        ...
        plot(z, Trajectory(6, :), 'o-')
        plot(z, Trajectory(7, :), 'o-')
        hold off
    end
    ...
    if FS
        ...
        for i = 1:PN-1
            for j = 1:PN-1
                if i == j %&& i ~= 6
                    plot(z, reshape(Xi(i, j, :), [NoP 1]), 'DisplayName', [num2str(i)  num2str(j)])
                elseif i == 6 && j == 6
                    disp('asd')
                    Xi(i, j, 7:32) = linspace(Xi(i, j, 6), Xi(i, j, 33), 32-7+1);
                    plot(z, reshape(Xi(i, j, :), [NoP 1]), 'o-', 'DisplayName', [num2str(i)  num2str(j)])
                else
                    plot(z, reshape(Xi(i, j, :), [NoP 1]), 'k')
                end
            end
        end
        legend
...
STDMC = load('Standard7particleSplitting.mat');
