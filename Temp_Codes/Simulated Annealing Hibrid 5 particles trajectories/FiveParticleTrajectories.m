...
FGS         = 0;        %figure switch 0 - figures off; 1 - figures on
IterData    = struct();
...
for state = 1:length(alphavals)
    ...
    iter = 1.025 * 10^7;    %# of iterations for one specific alpha
    ...
    for i = 1:iter
        pos_new     = HibridStep5Part(Position, alpha, eta, NoP, Nt, sigma(i), Exclude, p_in, p_out, z);
    ...
    save(['TrajectoryData_5p_' num2str(state)], "IterData");
end
