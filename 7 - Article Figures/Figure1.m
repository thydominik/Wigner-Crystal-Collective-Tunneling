...
xlabel('\chi', 'FontSize', 25)
    ...
    ylabel('V(\chi)', 'FontSize', FontSize)
    ...
    shift1 = -9.5;
    mult = 100;
    plot(x(abs(WF1) > trshld), shift1 + mult * abs(WF1(abs(WF1) > trshld)), 'r-', 'LineWidth', 2.0)
    plot(x(abs(WF2) > trshld), shift1 + mult * abs(WF2(abs(WF2) > trshld)), 'r-', 'LineWidth', 2.0)
    yline(shift1, 'k--', 'LineWidth', 3)
    shift2 = 9.2;
    plot(x(abs(WF3) > trshld), shift2 + mult * abs(WF3(abs(WF3) > trshld)), 'r-', 'LineWidth', 2.0)
    yline(shift2, 'k--', 'LineWidth', 3)
    ax = gca;
    %ax.YColor = 'b';
    ax.FontSize = FontSize - 5;
    %ylabel('\Psi_j(\chi)', 'FontSize', FontSize)

    %xxx = linspace(0, ((Pol.Kappa(1) * 1) + 0.012 * (1.^2 - Pol.Alpha(AlphaInd)).^2) / 0.03, 100);
    %plot(ones(1,100), xxx, 'k-')
%yline(0, 'k-', 'LineWidth', 3)
axis square
hold off
%
...
ax.FontSize = 20;
surf(linspace(-7, 7, 100), Pol.Kappa, reshape(LDOS(alphaInd, :, :), [201 100]), 'EdgeColor', 'none', 'FaceColor', 'interp')
plot3(Pol.Polarization(alphaInd, :), Pol.Kappa, 10 * ones(26,201), 'white--', 'LineWidth', 5)
%plot3(linspace(-7, 7, length(Pol.Polarization(alphaInd, :))), 0 * ones(1, length(Pol.Kappa)), 10 * ones(26,201), 'k-', 'LineWidth', 1.5)
xlim([-5.5 5.5])
xticks([-5 -2.5 0 2.5 5])
yticks([-0.1 -0.05 0 0.05 0.1])
