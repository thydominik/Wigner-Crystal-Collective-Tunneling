...
new_alpha = [];
alphaa = [];
for a_ind = 21:5:121      %Loop for a set of trajectories, this might have to be changes if theres is another set of trajectories
    ...
    equilibrium_positions   = load('Eq_Pos_eta_20_particles_5');    % 4th column is the alpha value!    I know that this is called in every loop, it's fine for now.
    ...
    for i = 1:(2*N_interp)
        ...
        if i <= N_interp/2
            Fitted_VS_2(i) = fc2.b * (S_new - min(S))^2;
        ...
        % 2ND FOR THE QUARTIC:
        if i <= N_interp/2
            Fitted_VS_4(i) = fc1.b * (S_new^2 - min(S)^2)^2;
        elseif i > N_interp/2 && i <= (3/2 * N_interp)
